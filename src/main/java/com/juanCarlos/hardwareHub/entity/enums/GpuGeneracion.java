package com.juanCarlos.hardwareHub.entity.enums;

import com.juanCarlos.hardwareHub.entity.GpuEntity;
import lombok.AllArgsConstructor;
import lombok.Getter;

/**
 * Contiene valores normalizados sobre las posibles generaciones a las que puede pertenecer una tarjeta gráfica.
 *
 * @author Juan Carlos
 * @see GpuEntity
 */

@AllArgsConstructor
@Getter
public enum GpuGeneracion implements DescribedEnum {

    GPU_GENERACION_RTX_4000("RTX 40"),
    GPU_GENERACION_RTX_5000("RTX 50"),
    GPU_GENERACION_RX_7000("RX 7000"),
    GPU_GENERACION_RX_9000("RX 9000"),
    GPU_GENERACION_ARC_SERIE_A("Arc Serie A"),
    GPU_GENERACION_ARC_SERIE_B("Arc Serie B");

    private String desc;
}